    ...
    <tbody>
        ...
        <c:forEach items="${requires}" var="require">
            <tr>
                ...
                <td>
                    <c:choose>
                        <c:when test="${require.status eq 3}">
                ...
                <td class="hidden-480">
                    <div class="hidden-sm hidden-xs btn-group">
                        ...
                        <%--</shiro:hasPermission>--%>
                            ...
                            <button class="btn btn-xs btn-info editQuestion"   data="${require.id}">
                                <i class="ace-icon fa fa-pencil bigger-120"></i>
                            </button>
                            <a class="btn btn-xs btn-danger delete"  data="${require.id}">
                                <i class="ace-icon fa fa-trash-o bigger-120"></i>
                            </a>
                            <button class="btn btn-xs btn-warning upToHot" data="${require.id}">
                                <i class="ace-icon glyphicon glyphicon-hand-up bigger-120"></i>
                            </button>
                            <button class="btn btn-xs btn-warning backToNormal" data="${require.id}">
...
<script>
    $(function(){
        $(".add").on("click",function(){
             window.location.href="${ctx}/require/add"
        });
        ...
        $(".delete").on("click",function(){
            ...
            if(r){
                 var id = $(this).attr("data");
                 window.location.href="${ctx}/require/delete?id="+id;
            }

        });
        $(".upToHot").on("click",function(){
            var id = $(this).attr("data");
            window.location.href="${ctx}/require/hot?id="+id;
        });
        $(".backToNormal").on("click",function(){
            var id = $(this).attr("data");
            window.location.href="${ctx}/require/back?id="+id;
        });
